...
// Memory + MemoryType moved to src/memory.rs so the wasm32 worker side
// (worker_store.rs) can share the same types. Re-exported here so the
// many existing `store::Memory` / `store::MemoryType` references keep
// working unchanged.
pub use crate::memory::{Memory, MemoryType};

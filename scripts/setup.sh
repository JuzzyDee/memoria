...
if [ ! -f wrangler.toml ] || [ ! -f Cargo.toml ]; then
    err "Run this from the memoria repo root (where wrangler.toml and Cargo.toml live)."
    exit 1
fi
...
if [ "$DRY_RUN" = true ]; then
    ...
    wrangler vectorize create memoria-vectors --dimensions=768 --metric=cosine 2>&1 | tail -3 \
        || warn "Vectorize index may already exist (proceeding)"
fi
...
if [ "$DRY_RUN" = true ]; then
    ...
    wrangler r2 bucket create memoria-images 2>&1 | tail -3 \
        || warn "R2 bucket may already exist (proceeding)"
fi
...
if [ "$DRY_RUN" = true ]; then
    ...
    wrangler d1 migrations apply memoria-db --remote 2>&1 | tail -5
fi
